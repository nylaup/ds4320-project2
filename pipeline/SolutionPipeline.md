...
try: 
    ...
    logger.error(f"Error occurred while fitting Linear Regression: {e}")
    raise e
...
try:
    ...
    logger.error(f"Error occurred while fitting Random Forest: {e}")
    raise e
...
try:
    ...
    logger.info("Random Forest Demographics fitted successfully")
except Exception as e:
    logger.error(f"Error occurred while fitting Random Forest Demographics: {e}")
    raise e
...
print(f"Random Forest Demographics | MAE: {mae_rf2:.3f}, RMSE: {rmse_rf2:.3f}, R^2: {r2_rf2:.3f}")
logger.info(f"Random Forest Demographics | MAE: {mae_rf2:.3f}, RMSE: {rmse_rf2:.3f}, R^2: {r2_rf2:.3f}")
#significantly worse performance without election history 
```
...
The model's final feature importance was greatly reliant on the 2012 election margins This shows that election outcomes are incredibly difficult predict and election forecasting remains a difficult task, even if we look at various demographic changes and historical results. Demographic change is less significant in predicting election margins than past history in elections, and these alone could not be used as predictors, as is seen from the final model that is trained without historical data and performs poorly. 
...
try: 
    ...
    ma_merged.plot(
        ...
        edgecolor="black",
        legend=True,
        legend_kwds={"label": "Margin (gop-dem pct diff)"},
        ax=ax
    )
...
ax.set_title("Massachusetts Counties Predicted Percent\n Margins for Future Election")
